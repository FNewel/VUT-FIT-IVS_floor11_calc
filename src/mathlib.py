...
def fact(n):
    ...
    if n % 1 != 0: raise TypeError 
    result = 1
    for i in range(1, int(n) + 1): result *= i
    return result
...
def root(n,x):
    ...
    if n < 0 or ((n % 2 == 0) and (x < 0)): raise ValueError
    if n % 1 != 0: raise TypeError
    if (n % 2 == 1) and (x < 0): return -(abs(x) ** (1/n))
    return x ** (1/n)
...
def rng(x):
    ...
    if (x % 1 != 0): raise TypeError
    if x > 0: return randint(0,x)
    return randint(x,0)
